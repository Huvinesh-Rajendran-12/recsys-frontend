...
func getSites() ([]Site, error) { // Update return type
	err := godotenv.Load()
	if err != nil {
		log.Fatal(err)
	}
	...
	err = json.NewDecoder(resp.Body).Decode(&sitesResponse)
	if err != nil {
		log.Println("Error decoding JSON response:", err)
		return nil, err // Return error and empty slice
	}

	return sitesResponse.Sites, nil // Return secrets and no error
}
